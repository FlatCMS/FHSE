...
CONFIG_PATH="$BUILDER_DIR/examples/fhse-pc.env.example"
BASE_ISO_URL="https://releases.ubuntu.com/22.04/ubuntu-22.04.5-live-server-amd64.iso"
BASE_ISO_PATH="$DIST_DIR/ubuntu-22.04.5-live-server-amd64.iso"
...
Options:
  --config PATH      FHSE PC config file
  --base-iso PATH    Ubuntu Server amd64 base ISO
...
need_supported_host() {
  case "$(uname -s)" in
    Linux|Darwin)
      ;;
    *)
      echo "ERROR: this ISO builder currently supports Linux and macOS only." >&2
      exit 1
      ;;
  esac
}
...
if [ "$DOWNLOAD_BASE_ISO" -eq 1 ] || [ ! -f "$BASE_ISO_PATH" ]; then
  echo "Downloading Ubuntu Server base ISO..."
...
echo "Extracting Ubuntu base ISO..."
xorriso -osirrox on -indev "$BASE_ISO_PATH" -extract / "$ISO_ROOT_DIR" >/dev/null 2>&1
...
rm -f "$OUTPUT_SHA256_PATH"
echo "Building FHSE x86_64 ISO..."
xorriso \
  -indev "$BASE_ISO_PATH" \
...
echo "FHSE PC ISO built:"
echo "  $OUTPUT_ISO_PATH"
echo "FHSE PC ISO checksum:"
